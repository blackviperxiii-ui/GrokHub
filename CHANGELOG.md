...
- Track Grok Build CLI **1.0.31** alpha. `--help` and cabin spawn/parse (`grok -p --output-format streaming-json`, `agent stdio`, `sessions`, `export`, `update --alpha`) are unchanged from 1.0.30. Unpackaged `grok update --check --json` still reports `channel=stable` and `latestVersion=1.0.30` (npm `latest`); the cabin stays on alpha via `grok update --alpha` and does not treat the version number as the channel.
