...
pub const SLASH_COMMANDS: &[SlashDef] = &[
    ...
    SlashDef { cmd: "/project delete", hint: "Remove the project; chats return to History", insert: "/project delete", run_on_pick: true },
    SlashDef { cmd: "/board", hint: "Open Workboards", insert: "/board", run_on_pick: true },
...
pub fn slash_help() -> String {
    [
        ...
        "/project delete — remove the project; its chats return to History",
        ...
        "Settings → Behavior: close to tray, living wall, and a quiet hours dropdown. Picking a window saves it. A Quiet until chip shows on the titlebar only while that window is active. Signed-in empty home shows an update feed under the greeting when a card is undismissed, and hides that slot when the feed is empty. A finished /loop posts automation_done from poll_grok_loop. Saving a schedule posts schedule_created. Cards stay until opened or dismissed. A device glance appears only when hub share or a last frame is bound; click opens Devices.",
        ...
        "Appearance: Dark, Light, System. Ask, Auto, and Always keep one grok agent stdio (ACP) session so a background task, monitor, or loop stays up after the turn. Allow / Deny shows for Ask. If ACP is down the turn is denied. Auto and Always answer permission prompts and inherit the PermissionMode pill. `/workflow` `/compact` `/rewind` honor that same pill — Ask fail-closed if ACP is down, Auto/Always keep session mode. btw (saved as ask) is a side ask: a live run keeps going and the question waits, then sends look-safe on grok -p (`--permission-mode default`, no desktop-do-the-work). Idle btw sends that same look-safe ask. Night/inbox/anticipate inherit scheduled_args like loops — Ask is fail-closed, no ACP. Halt is session/cancel.",
...
mod tests {
    ...
    fn cabin_slash() {
        ...
        assert!(
            slash_help().contains("keep one grok agent stdio")
        ...
        assert!(slash_help().contains("Collapse leaves one short row"));
        assert!(!slash_help().contains("Hide stops drawing that thought"));
        assert!(slash_help().contains("The reply stays"));
        assert!(slash_help().contains("That fold survives the live-to-stored handoff"));
        assert!(slash_help().contains("A long user bubble wraps inside the row"));
        assert!(slash_help().contains("Copy and Reply under a user bubble stay inside the window"));
        assert!(slash_help().contains("keeps its leading gap"));
        assert!(slash_help().contains("Card and wall hover stay inside the slot"));
        assert!(slash_help().contains("Account sets a display name"));
        assert!(slash_help().contains("local profile picture"));
        assert!(slash_help().contains("do not show the email"));
        ...
        assert!(slash_help().contains("automation_done"));
        assert!(slash_help().contains("schedule_created"));
        assert!(!slash_help().contains("Coding / Life chip"));
        assert!(!slash_help().contains("interest_update"));
        assert!(slash_help().contains("device glance appears only when hub share"));
        assert!(slash_help().contains("hides that slot when the feed is empty"));
        assert!(slash_help().contains("leaves the rewind snapshot out"));
        assert!(slash_help().contains("History is the cabin's own chats"));
