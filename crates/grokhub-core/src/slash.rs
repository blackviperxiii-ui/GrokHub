...
pub fn slash_help() -> String {
    [
        ...
        "Voice: OAuth for STT/TTS; duplex streams PCM with a console key. Hands: install.sh/AUR pull ydotool; PATH includes ~/.local/bin; Eyes Install hands starts ydotoold; /stop kills the worker.",
...
mod tests {
    ...
    fn cabin_slash() {
        ...
        assert!(slash_help().contains("install.sh/AUR pull ydotool"));
