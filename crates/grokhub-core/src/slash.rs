...
pub const SLASH_COMMANDS: &[SlashDef] = &[
    ...
    SlashDef { cmd: "/update", hint: "Overlay from Origin when behind", insert: "/update", run_on_pick: true },
...
pub fn slash_help() -> String {
    [
        ...
        "/imagine <prompt>",
        "/update — overlay from Origin when behind (clean main clone); Restart on Settings (ydotoold → hub, then a new cabin process)",
