...
pub fn slash_help() -> String {
    [
        ...
        if cfg!(windows) {
            "First-time install (Windows Setup) installs Grok Build CLI alpha from https://x.ai/cli (GROK_CHANNEL=alpha). First run is Get Started — Super Grok OAuth signs in grok too. Settings → Install Grok Build CLI installs alpha when grok is missing or broken. Settings → Connect does the same when grok is not already connected. Settings → Update downloads the latest grokhub-windows zip from GitHub (or overlays a source clone), then runs grok update --alpha. If grok is on stable after an upgrade, the cabin switches it back to alpha. Settings shows grok --version."
        } else {
            "First-time install (install.sh, AUR) installs Grok Build CLI alpha from https://x.ai/cli (GROK_CHANNEL=alpha). First run is Get Started — Super Grok OAuth signs in grok too. Settings → Install Grok Build CLI installs alpha when grok is missing or broken. Settings → Connect does the same when grok is not already connected. Cabin overlay updates the GUI, then runs grok update --alpha. If grok is on stable after an upgrade, the cabin switches it back to alpha. It does not yank a working alpha install."
        },
