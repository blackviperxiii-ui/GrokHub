...
pub fn slash_help() -> String {
    [
        ...
        if cfg!(windows) {
            "First-time install and reinstall (Windows Setup + first cabin launch) automatically install Grok Build CLI alpha from https://x.ai/cli (GROK_CHANNEL=alpha). UAC on first run is expected. First run is Get Started — Super Grok OAuth signs in grok too. The Install control is hidden when grok is already present or an alpha install is already running. Settings → Update → Update Grok Build CLI runs grok update --alpha when grok is already installed. Settings → Connect writes CLI auth when grok is not already connected. Settings → Update overlays the cabin (GitHub zip when there is no clone), then runs grok update --alpha. A newer GitHub Latest cabin notifies in-app on Settings → Update (Update + Restart), not a web page. If grok is on stable after an upgrade, the cabin switches it back to alpha. Settings About shows grok --version."
        } else {
            "First-time install and reinstall (install.sh, AUR, first cabin launch) automatically install Grok Build CLI alpha from https://x.ai/cli (GROK_CHANNEL=alpha). First run is Get Started — Super Grok OAuth signs in grok too. The Install control is hidden when grok is already present or an alpha install is already running. Settings → Update → Update Grok Build CLI runs grok update --alpha when grok is already installed. Settings → Connect writes CLI auth when grok is not already connected. Cabin overlay updates the GUI, then runs grok update --alpha. A newer GitHub Latest cabin notifies in-app on Settings → Update (Update + Restart), not a web page. If grok is on stable after an upgrade, the cabin switches it back to alpha. It does not yank a working alpha install."
        },
