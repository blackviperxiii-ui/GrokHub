...
pub fn slash_help() -> String {
    [
        ...
        "Host, hands, and connector work stay off the chat. Chat shows each thought as its own bubble and the final reply. User bubbles sit on the right. Bubbles wrap with the chat pane. Copy and Reply sit on visible messages. Hands map JPEG clicks through xrandr; act picks the smallest AT-SPI match. A full-desktop frame stays at 0,0 — it does not inherit a single-monitor origin. tab list/close/focus talks to localhost CDP when it is up. GUI-help turns show a Hands chip and a how-to. Hung desktop tools time out so the cabin does not freeze. GitHub, CDP, chat SSE, and OAuth JSON bodies are capped. Chat-complete text is clipped before strip/merge. Live thought+stream merge stays capped; leftover deltas skip upsert. Chat bubble layout clips paint to TEXT_FILE_CAP. Stream deltas refresh the trailing chat view without cloning the thread; ChatView bodies stay inside TEXT_FILE_CAP.",
