...
mod tests {
    ...
    fn get_started_and_cli_sync_predicates() {
        ...
        // Live unpackaged 1.0.35: version is alpha, update channel is still stable
        // until config / `grok update --alpha`. Version number is not the channel.
        // latestVersion is npm latest (1.0.34).
        let unpackaged_1035 = parse_cli_update_check_channel(
            r#"{"currentVersion":"1.0.35","latestVersion":"1.0.34","updateAvailable":false,"installer":"internal","channel":"stable","autoUpdate":null,"error":null}"#,
        );
        assert_eq!(unpackaged_1035.as_deref(), Some("stable"));
        assert!(should_switch_cli_to_alpha(unpackaged_1035.as_deref()));
        assert_eq!(
