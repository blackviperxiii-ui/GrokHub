...
Composer placeholder: **What do you want to know?** Mode pill: Auto / Fast / Balance / Think / Max. Auto routes from the ask (`grok-3-mini-fast` / `grok-4.3` / `grok-4.7` `high` / `grok-4.7` `xhigh`). A Settings chat-model pin skips Auto only when it is not a ladder default — `/mode` does not write that pin. Fast pins mini. Balance is Grok 4.3. Think is Grok 4.7 at high. Max is Grok 4.7 at xhigh. Failover on 401/403/429/5xx: 4.7 → 4.3 → Fast. `/mode auto|fast|balance|think|max`. The composer Effort dropdown does not offer Max; a saved Max effort loads as Extra High.
...
Cabin panes: Chat, Devices, Memory, Board, Imagine, Skills, Eyes, Settings. Left rail Chat sits above Imagine. Chat reuses one empty draft (no dialogue); after a reply starts it opens a new chat. Old convos are sidebar History, not a second New chat button. Left rail also holds the project tree.
