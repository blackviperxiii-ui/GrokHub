...
Bearer for `api.x.ai` is: console API key if set, else OAuth access token. Refresh 30 minutes before expiry. Tokens live in `~/.config/GrokHub/secrets.json` mode `0600` (Windows user-only DACL). Never in markdown. The cabin footer paints the Grok profile photo when the session has one.
