...
Later updates: Settings → **Update**, `/update`, or `grokhub --update`. The clone must be on `main` with an `origin`. Overlay plus `grok update --alpha` — `~/.config/GrokHub` stays. If `grok` is on stable, the overlay switches it to alpha. Progress stays on Settings. After a clean overlay, **Restart** reloads the new binary.
