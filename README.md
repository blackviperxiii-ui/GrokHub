...
Projects sit in the left rail. `+` makes a project (`~/GrokHub-Work/<slug>`) or a one-level folder. Double-click or right-click to rename (display name only — the path stays). Right-click a project to add it to a folder or remove it. Folders are sidebar only; they do not move files. Click a project to bind it and reopen the project chat you left. That click does not filter, hide, or remove the chat section. Creating a project does not wipe History. Right-click a folder or project and choose New chat to file another chat there; the projects stay in the folder. Click the project again, while you are already in its chat, and new chats go to History. Delete puts those chats back in History and does not wipe transcripts. A project click does not open the Workboard. Bound tree is the world.
