...
**v2.9.4** — Grok Build **1.0.21**. Chat is full-pane bubbles; thinking is faded thought process; tool work starts collapsed. First run is Get Started (Super Grok OAuth signs in `grok`). Get Started installs Grok Build CLI when `grok` is missing or broken. Hover composer pills for what they do. Windows Update downloads the GitHub zip (no clone). Linux `/update` is `grok update` (no `--alpha`). History is `grok sessions` 1:1.
...
Get Started and Settings → **Update** → **Install Grok Build CLI** install Grok Build CLI **alpha** (`GROK_CHANNEL=alpha` / `https://x.ai/cli/alpha`) when `grok` is missing or broken (a leftover that cannot start counts as broken). First launch does the same automatically. A leftover `grok.exe` that cannot start (missing DLL) shows one cabin error, not a looping Windows dialog.

Settings → **Update** (or `grokhub --update` / `/update`) is **Install Grok Build CLI** when grok is missing or unusable, the install-overlay control, **Update**, progress, and **Restart** after a clean overlay. It retargets a leftover Origin or `GrokHub-Windows` clone to GitHub (`https://github.com/blackviperxiii-ui/GrokHub.git`), then `git pull --ff-only origin main`. Linux overlays with `./scripts/install.sh --user` and `grok update` on the current channel (no `--alpha`). Windows Setup users do not need a clone: Update downloads the latest `grokhub-windows-v*.zip` from GitHub into `%LOCALAPPDATA%\Programs\GrokHub`, then runs `grok update --alpha` (PATH includes `%USERPROFILE%\.grok\bin`). A Windows source clone on `main` still overlays with `scripts/install-windows.ps1`. The clone path is not a Settings field — empty uses `GROKHUB_SRC` or the install receipt. After a clean overlay, **Restart** reloads hub, drops the cabin pid lock, starts a new overlay `grokhub`, and exits this process.
