...
Composer session pills: **Chat** / **Plan** / **Ask**. Permission: **Ask** / **Auto** / **Always**. Effort dropdown: **Low** / **Medium** / **High** / **Extra High** (`grok agent --reasoning-effort`). `/effort` sets the same. Grok Build chooses models. Empty-home greeting and quick chips use **Grok 4.1 Fast** (`grok-4-1-fast-non-reasoning`) through `grok login`.
...
History lists cabin chats plus `grok sessions` (and `~/.grok/sessions`). Click a Grok session to resume it; transcripts load via `grok export`. Each cabin thread stores the ACP session id so history survives the grok CLI backend. Alpha channel: `grok update --alpha`.
